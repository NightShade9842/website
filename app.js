...
window.addEventListener('DOMContentLoaded', () => {
    // Only run auto-login on index.html
    ...
    if (document.getElementById('board-content')) loadBoardContent();
...
async function loginWithToken(token) {
    try {
        ...
        if (data.token) {
            ...
            alert(data.error || 'Invalid or expired link. Get a new .login link from the bot.');
            document.getElementById('auto-login')?.classList.add('hidden');
            document.getElementById('no-token')?.classList.remove('hidden');
        }
    } catch (e) {
        alert('Connection error. Make sure the API is deployed correctly.');
        document.getElementById('auto-login')?.classList.add('hidden');
        document.getElementById('no-token')?.classList.remove('hidden');
    }
}
...
async function buyCard(listingId) {
    const res = await apiFetch('/shop/buy', { method: 'POST', body: JSON.stringify({ listingId }) });
    if (res?.success) { alert('Card purchased!'); switchTab('cards'); }
...
async function placeBid(auctionId) {
    ...
    const res = await apiFetch('/auctions/bid', { method: 'POST', body: JSON.stringify({ auctionId, amount: parseInt(amount) }) });
    if (res?.success) { alert('Bid placed!'); switchTab('auctions'); }
    else alert(res?.error || 'Bid failed');
}
...
async function loadEventStore(container) {
    ...
    container.innerHTML = items.map(item => `
        <div class="glass rounded-xl p-4 flex items-center justify-between mb-3">
            <div><p class="font-bold">${item.name}</p><p class="text-sm text-gray-400">${item.description}</p><p class="text-yellow-400">🪙 ${item.price_coins} Gold Coins</p></div>
...
async function buyEventItem(itemId) {
    const res = await apiFetch('/events/buy', { method: 'POST', body: JSON.stringify({ itemId }) });
    if (res?.success) { alert('Purchased!'); switchTab('events'); }
    else alert(res?.error || 'Purchase failed');
}
...
async function loadItemStore(container) {
    ...
    container.innerHTML = items.map(item => `
        <div class="glass rounded-xl p-4 flex items-center justify-between mb-3">
            <div><p class="font-bold">${item.name}</p><p class="text-sm text-gray-400">${item.description}</p>
                <div class="flex space-x-4 text-sm">${item.price_gems ? `<span class="text-blue-400">💎 ${item.price_gems} Gems</span>` : ''}${item.price_beli ? `<span class="text-yellow-400">💰 ${Number(item.price_beli).toLocaleString()} Beli</span>` : ''}</div></div>
...
async function buyStoreItem(itemId) {
    const res = await apiFetch('/store/buy-item', { method: 'POST', body: JSON.stringify({ itemId }) });
    if (res?.success) { alert('Item purchased!'); switchTab('items'); }
    else alert(res?.error || 'Purchase failed');
}
...
async function loadPokeMart(container) {
    ...
    if (!items || !items.length) { container.innerHTML = '<p class="text-gray-400">PokéMart is empty.</p>'; return; }
    container.innerHTML = items.map(item => `
        <div class="glass rounded-xl p-4 flex items-center justify-between mb-3">
            <div><p class="font-bold">${item.name}</p><p class="text-sm text-gray-400">${item.description}</p><p class="text-yellow-400">💰 ${Number(item.price_beli).toLocaleString()} Beli</p><p class="text-xs text-gray-500">Qty: ${item.quantity_per_purchase}</p></div>
...
async function buyPokeMartItem(itemId) {
    const res = await apiFetch('/store/buy-pokemart', { method: 'POST', body: JSON.stringify({ itemId }) });
    if (res?.success) { alert('Purchased!'); switchTab('pokemart'); }
    else alert(res?.error || 'Purchase failed');
}
...
async function loadGuildStore(container) {
    ...
    if (!items || !items.length) { container.innerHTML = '<p class="text-gray-400">Guild store is empty.</p>'; return; }
    container.innerHTML = items.map(item => `
        <div class="glass rounded-xl p-4 flex items-center justify-between mb-3">
            <div><p class="font-bold">${item.name}</p><p class="text-sm text-gray-400">${item.description}</p><p class="text-purple-400">💨 ${item.price_dust} Dust</p></div>
...
async function buyGuildItem(itemId) {
    const res = await apiFetch('/store/buy-guild', { method: 'POST', body: JSON.stringify({ itemId }) });
    if (res?.success) { alert('Purchased!'); switchTab('guild'); }
    else alert(res?.error || 'Purchase failed');
}
...
async function loadHatchery(container) {
    ...
    if (!items || !items.length) { container.innerHTML = '<p class="text-gray-400">Hatchery is empty.</p>'; return; }
    ...
    container.innerHTML = items.map(item => {
        ...
        return `<div class="glass rounded-xl p-4 flex items-center justify-between mb-3"><div><p class="font-bold">${rarityEmoji[item.egg_rarity] || '🥚'} ${item.name}</p><p class="text-sm text-gray-400">${item.description}</p><p class="text-yellow-400">${priceText}</p><p class="text-xs text-gray-500">🐣 ${item.egg_rarity} Pokémon (×${item.pokemon_count})</p></div><button onclick="buyHatcheryItem(${item.id})" class="bg-pink-500 px-4 py-2 rounded-lg text-white font-semibold hover:bg-pink-400">Buy</button></div>`;
    }).join('');
}

async function buyHatcheryItem(itemId) {
    const res = await apiFetch('/store/buy-hatchery', { method: 'POST', body: JSON.stringify({ itemId }) });
    if (res?.success) { alert('Egg added!'); switchTab('hatchery'); }
    else alert(res?.error || 'Purchase failed');
}

// ===== LEADERBOARD =====
let currentLB = 'level';
function switchLB(type) { currentLB = type; document.querySelectorAll('.lb-tab').forEach(b => b.classList.remove('tab-active')); document.getElementById('lb-' + type)?.classList.add('tab-active'); loadLeaderboard(); }
async function loadLeaderboard() {
    ...
    if (Array.isArray(data)) {
        container.innerHTML = `<table class="w-full text-left"><thead><tr class="text-gray-400 border-b border-gray-700"><th>#</th><th>Pirate</th><th>Stats</th></tr></thead><tbody>${data.map((e,i) => `<tr class="border-b border-gray-800"><td class="py-2">${i+1}</td><td class="py-2">${e.pirate_name || e.name || 'Unknown'}</td><td class="py-2">${currentLB==='level'?`Level ${e.level}`:currentLB==='richest'?`💰 ${(e.total||e.beli+e.bank)?.toLocaleString()}`:currentLB==='legendary'?`${e.count} Legendary`:`${e.treasury?.toLocaleString()}`}</td></tr>`).join('')}</tbody></table>`;
    } else container.innerHTML = '<p class="text-red-400">Failed to load</p>';
}